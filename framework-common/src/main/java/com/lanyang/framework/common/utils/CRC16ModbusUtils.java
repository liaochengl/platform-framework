 ...
 * @des
 */
public class CRC16ModbusUtils {

    /**
     * 校验
     * @param source 源校验码
     * @param target 需要生成校验码的数组
     * @return
     */
    public static boolean verify(String source, String...target){

        return StringUtils.equalsIgnoreCase(byteTo16String(getVerifyValue(target)).replace(" ", ""), source);
    }

    /**
     * 获取源数据和验证码的组合byte数组
     *
     * @param target 可变长度的十六进制字符串
     * @return
     */
    public static byte[] getVerifyValue(String...target) {
        byte[] data = new byte[]{};
        for (int i = 0; i<target.length;i++) {
            int x = Integer.parseInt(target[i], 16);
            byte n = (byte)x;
     ...
     * @return
     */
    private static byte[] getVerifyValue(byte[] aa) {
        byte[] bb = getCrc16(aa);
        return bb;
    }
     ...
     * [-128,-1],0,[1,128)
     */
    private static String byteTo16String(byte b) {
        StringBuffer buffer = new StringBuffer();
        int aa = (int)b;
        if (aa<0) {
